...
def torsional_oscillator_model(x, C, damp, I, kappa, G, phi):
    ...
    w1 = np.sqrt(w0**2 - b**2)
    theta = [C*np.e**(-b*t)* np.cos(w1*t+phi) for t in x]
    return theta 
...
def curve_fitting(x, y, y_err, x_title, y_title, model, linecolors = ["#FF9E00", "#00965B", "#0A4A70", "#021D27", "#EF476F"], 
    ...
    def linear_regression_with_errors(x, y, y_err, model):
        popt, pcov = curve_fit(model, xdata = x, ydata = y, sigma=y_err, absolute_sigma=True)
